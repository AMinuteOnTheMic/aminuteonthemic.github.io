(function(){ 
  ...
  (function(){
    ...
    if(links) {
      Array.from(links).forEach(function(link){
        ...
        isExternal = (url && typeof url == 'string' && url.startsWith('http')) && !containsClass(link, 'nav_item') && !isChild(link, ['.archive', '.article', '.episode_nav', '.pager']) ? true : false;
        if(isExternal) {
          target = 'target';
          rel = 'rel';
          blank = '_blank';
  ...
  (function copyLinkToShare() {
    let  copy, copied, excerpt, isCopyIcon, isInExcerpt, link, page, episodeCopy, episodeLink, target;
    copy = 'copy';
    copied = 'copy_done';
    excerpt = 'excerpt';
    episodeCopy = 'episode_copy';
    episodeLink = 'episode_card';
    page = document.documentElement;
    
    page.addEventListener('click', function(event) {
      target = event.target;
      isCopyIcon = containsClass(target, copy);
      isInExcerpt = containsClass(target, episodeCopy);
      if (isCopyIcon) {
        if (isInExcerpt) {
          link = target.closest(`.${excerpt}`).previousElementSibling;
          link = containsClass(link, episodeLink)? elemAttribute(link, 'href') : false;
        } else {
  ...
  (function hideAside(){
    let aside, title, episodes;
    aside = elem('.aside');
    title = aside ? aside.previousElementSibling : null;
    if(aside && title.nodeName.toLowerCase() === 'h3') {
      episodes = Array.from(aside.children);
      episodes.length < 1 ? title.remove() : false;
    }
  })();
